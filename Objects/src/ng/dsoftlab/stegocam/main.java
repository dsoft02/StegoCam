...
public class main extends androidx.appcompat.app.AppCompatActivity implements B4AActivity{
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "ng.dsoftlab.stegocam", "ng.dsoftlab.stegocam.main");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "ng.dsoftlab.stegocam.main", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
...
vis = vis | (decode.mostCurrent != null);
...
vis = vis | (stegopreview.mostCurrent != null);
return vis;}
...
public ResumableSub_Activity_Create(ng.dsoftlab.stegocam.main parent,boolean _firsttime) {
this.parent = parent;
...
//if
this.state = 6;
if (parent._firstrun || (parent.mostCurrent._userpin).equals("")) { 
this.state = 3;
}else {
this.state = 5;
}if (true) break;
...
encode._process_globals();
history._process_globals();
decode._process_globals();
settings._process_globals();
pinreset._process_globals();
forgotpin._process_globals();
login._process_globals();
pinchange._process_globals();
pinsetup._process_globals();
securityquestion._process_globals();
stegopreview._process_globals();
